...
import {serviceInfo, aboutList, getNavigation} from '~/api/main'
...
export default {
    ...
    return new Promise((resolve, reject) => {
      aboutList().then(res => {
        if (res.data.code == 200) {
          let now = new Date();
          let data = {time: now.getTime(), list: res.data.data.bottomNavigationList};
  ...
  GET_NAV (store) {// 获取导航信息
    let now = new Date();
    if (CACHED.has('navigationList')) {
      const data = CACHED.get('navigationList');
      const result = JSON.parse(data);
      console.log(' 缓存命中2')
      // console.log(data)
      if (now.getTime() - result.time < ttl - 10000) {
        // 确保不超出缓存时间
        store.state.navigationList = result;
        return Promise.resolve(data)
      }
    }
    return new Promise((resolve, reject) => {
      getNavigation().then(res => {
        if (res.data.code == 200) {
          ...
          store.state.navigationList = res.data.data;
          CACHED.set('navigationList', JSON.stringify(res.data.data), ttl);
        }
        resolve(res.data)
      }).catch(error => {
        reject(error)
