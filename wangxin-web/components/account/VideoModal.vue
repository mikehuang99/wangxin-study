      ...
      <div class="modal_body">
        ...
        <div class="image_list">
          <div class="left_list">
            ...
            <ul class="">
              <li class="img" v-for="(item, index) in videoList" :key="index">
                <div class="oper">
                  <i class="iconfont gb" style="text-decoration:underline;" title="从库移除" @click="delVideo(item.videoLocalNumber, 1)">删除</i>
                </div>
          ...
          <div class="right_list">
            <div data-v-1300c342="" class="title">视频库列表<br/>(选中点击"确定"后并且审核之后下次打开该窗口左边"已选视频"才显示所选)</div>
            <ul>
              <li class="img" v-for="(item, index) in chapterVideoList" :key="index" @click="selVideo(item)" title="点击选择">
                <div class="oper">
                  <i class="iconfont gb" style="text-decoration:underline;" title="从库移除" @click.stop="delVideo(item.videoLocalNumber, 0, item.id)">删除</i>
                </div>
                <div class="icon iconfont" :title="item.videoName">☆</div>
                <a href="javascript:" :title="item.videoName">{{item.videoName}}</a>
              </li>
...
import {teacherFreshCourseVideoUpdate, teacherFreshCourseVideoSave, teacherFreshCourseVideoList,queryCourseVideoByLessonId, teacherFreshCourseVideoDelete} from '~/api/account/course.js'
export default {
  props: {
    open: {
      type: Boolean,
  ...
  methods: {
    ...
    savaVideo (vid, tit) {
      // console.log(vid)
        teacherFreshCourseVideoSave({
        chapterId: this.data.cNo,
        videoLocalNumber: vid
      }).then(res => {
        res = res.data;
        console.log(res)
        if (res.code === 200) {
      ...
      if (ty) {
        this.$msgBox({
          ...
          for (let i = 0; i < this.videoList.length; i++) {
            if (this.videoList[i].videoLocalNumber === vNo) {
        ...
        if (this.flag) {*/
          this.$msgBox({
            content: '你确定要删除该视频吗?'
              ...
              teacherFreshCourseVideoDelete({id}).then(res => {
              res = res.data;
              // console.log(res)
              if (res.code === 200) {
    ...
    // 提交保存选中视频
    submit () {
      let videoLocalNumber = this.videoList && this.videoList.length ? this.videoList[0].videoLocalNumber : ''
      console.log({videoLocalNumber, lessonId: this.data.pNo})
        teacherFreshCourseVideoUpdate({videoLocalNumber, lessonId: this.data.pNo}).then(res => {
        res = res.data;
        console.log(res)
        console.log('save====')
        if (res.code === 200) {
    ...
    changeTab (int) {
      ...
      // 获取该例题已选中的图片
    lessonVideoList () {
        queryCourseVideoByLessonId({lessonId: this.data.pNo}).then(res => {
        console.log(res)
        res = res.data
        console.log('lesson===')
