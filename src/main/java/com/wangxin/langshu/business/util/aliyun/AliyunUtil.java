...
public final class AliyunUtil {
	...
	public static String getUrlSign(Aliyun aliyun, String url, Date expires) {
		GeneratePresignedUrlRequest generatePresignedUrlRequest = new GeneratePresignedUrlRequest(aliyun.getAliyunOssBucket(), url.replace(aliyun.getAliyunOssBucketUrl(), ""));
	...
	public static File download(Aliyun aliyun, String url) {
		String key = check(url, aliyun.getAliyunOssBucketUrl());
...
*/
	public static String uploadPic(PlatformEnum platformEnum, File file, Aliyun aliyun) {
		...
		try {
			...
			return aliyun.getAliyunOssBucketUrl() + filePath;
		} catch (Exception e) {
			log.error("上传失败", e);
			return "";
		}
	...
	public static String uploadDoc(PlatformEnum platformEnum, File file,String originalFileNamePre, Aliyun aliyun) {
		// 上传
		try {
			String name = file.getName();
			String filePath = platformEnum.name().toLowerCase() + "/" +originalFileNamePre + "_"+ StrUtil.get32UUID() + name.substring(name.lastIndexOf("."));
	...
	public static void delete(String url, Aliyun aliyun) {
		try {
			String filePath = url.replace(aliyun.getAliyunOssBucketUrl(), "");
	...
	public static String uploadOAS(File file, Aliyun aliyun) {
		// 上传文件
		try {
			UploadResult uploadResult = getArchiveManager(aliyun.getAliyunAccessKeyId(), aliyun.getAliyunAccessKeySecret(), aliyun.getAliyunOasUrl()).upload(aliyun.getAliyunOasVault(), file);
	...
	public static boolean sendMsg(String phone, String code, Aliyun aliyun) throws ClientException {
		...
		request.setTemplateParam("{\"code\":\"{code}\",\"product\":\"wangxinvip\"}".replace("{code}", code));
