...
@Repository
public class PointOperateRecordDao extends BaseDao {
	...
	public Page<PointOperateRecordVo> queryPointOperateRecords(PointOperateRecordQ q) {
        ...
        sb.append("select q.* from point_operate_record q  where 1=1 ");
        
    	if(StringUtils.isNotEmpty(q.getUserNoOfPoint())){
    		sb.append(" and q.user_no_of_point = :user_no_of_point ");
    		paramMap.put("user_no_of_point", q.getUserNoOfPoint());
    	}
        ...
        List<PointOperateRecordVo> list = new ArrayList<PointOperateRecordVo>();
        for(Map<String, Object> map:pge.getList()){
        	...
        	PointOperateRecordVo vo = new PointOperateRecordVo();
        	ObjectMapUtil2.mapToObjModle(vo,mapHump);
        	list.add(vo);
        }
        
        Page<PointOperateRecordVo> pg = new Page<PointOperateRecordVo>();
