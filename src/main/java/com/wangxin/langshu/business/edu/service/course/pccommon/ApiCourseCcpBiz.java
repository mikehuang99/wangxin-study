...
import com.wangxin.langshu.business.edu.dao.course.jdbc.CourseCcpDao;
 ...
 */
@Service
public class ApiCourseCcpBiz implements IApiCourseCcpBiz{

	@Autowired
	private CourseCcpDao courseCcpDao;
	...
	public CourseCcpListVo list(CourseCcpQ courseCcpQ) {
		...
		List<Map<String, Object>> ccpMaps = courseCcpDao.queryCcp(courseCcpQ);
	...
	/*public CourseCcpListVo list(CourseCcpQ courseCcpQ) {
		...
		for(CourseCcpVo courseCcpVo:courseCcpVosList){
			...
			for(Map<String, Object> map : ccpMaps){
				...
				//}
				if(map.get("course_id").toString().equalsIgnoreCase(String.valueOf(courseId))){
					String chapterId = map.get("chapter_id").toString();
					if(!chapterIds.contains(chapterId)){
						...
						for(Map<String, Object> m : ccpMaps){
							if(m.get("chapter_id").toString().equalsIgnoreCase(String.valueOf(chapterId))){
								if(null!=m.get("lesson_id")){
									String lessonId = m.get("lesson_id").toString();
