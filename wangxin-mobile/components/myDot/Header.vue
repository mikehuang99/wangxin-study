<template>
  <header class="header" :class="[this.backgroundColor ? 'fff' : '' ]">
    ...
    <span style="font-size: 18px;font-weight: bold;color: white">点券记录</span>
    <div style="width: 30%;height: 90%;position: absolute;right: 10px;
    color: #ffffff;font-size: 14px;top: 12px;text-decoration: underline" @click="openAddPointDialog">兑换积分</div>
  </header>
</template>
<script>
export default {
  ...
  methods: {
      openAddPointDialog(){
          this.$parent.openAddPointDialog();
      },
    go () {
      this.$router.go(-1)
    },
