      ...
      <div tag='li' :data-id="item.id" :key="index" v-for="(item, index) in studyLogArr">
        <div class="course_child_list n">
          <div class="course_child_con" >
            <div class="course_content_box point_sty">
                ...
                <li class="course_title_h1">
                  <h1>
                    <i>[{{index+1}}].<span style="font-weight: bold;color: red;font-size: 18px" v-if="item.operateType==0">增加</span>
                      <span style="font-weight: bold;color: red;font-size: 18px" v-if="item.operateType==1">减少</span>积分:{{item.point}},积分总量{{item.totalPoint}}</i>
									</h1>
                </li>
                <div>
		...
		<p class="no_data" v-show="this.touchend">没有更多积分记录</p>
...
import {pointOperateRecordList} from '~/api/market'
import { logout } from '~/store/common'
import Footer from '~/components/myPoint/Footer.vue'
...
export default {
  ...
  methods: {
    ...
    findPointList(offset) {
        ...
        pointOperateRecordList({'pageCurrent':this.page.pageCurrent+1,'pageSize':this.page.pageSize
               }).then(res => {
