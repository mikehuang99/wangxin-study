  ...
  <div>
    ...
    <ul v-if="studyLogArr.length">
      <div tag='li' :data-id="item.id" :key="index" v-for="(item, index) in studyLogArr">
            ...
            <div class="course_content_box coupon_sty">
              <ul class="context_area on" style="width:100%;padding-left:6px;">
                ...
                <div>
                  ...
                  <div v-if="!isHadCollected(item.id)" @click="collectCoupon(item.id)" style="position: relative;float:right;right:0px;border-radius:3px;bottom:80px;border:solid 1px red;font-size: 18px;font-weight:bold;width: 60px;height: 30px;text-align: center;background-color: #7FFFD4">
                    领取
                  </div>
                  <div v-if="isHadCollected(item.id)" style="position: relative;float:right;right:0px;border-radius:3px;bottom:80px;border:solid 1px red;font-size: 18px;font-weight:bold;width: 60px;height: 30px;text-align: center;background-color: lightgrey">
                    已领取
                  </div>
                </div>
              </ul>
		...
		<p class="no_data" v-show="this.touchend">没有更多优惠券可以领取</p>
...
import {canCollectCouponsList,saveCouponUser} from '~/api/market'
import { logout } from '~/store/common'
import Footer from '~/components/collectCoupon/Footer.vue'
...
export default {
    ...
    return {
        ...
        hadCollectedCouponIds: [],//已经领取d优惠券的ID
        touchend: false,
        studyLogArr: [],
  ...
  methods: {
    ...
    initData () {
      ...
      isHadCollected(couponId){
          let count =  0;
          for(let i=0;i<this.hadCollectedCouponIds.length;i++){
              if(this.hadCollectedCouponIds[i]==couponId){
                  count ++;
              }
          }
          if(count==0){
      ...
      collectCoupon(couponId) {
          ...
          saveCouponUser({couponId: couponId}).then(res => {
              ...
              if (res.code === 200) {
                  //this.getCouponList();
                  this.hadCollectedCouponIds.push(couponId);
