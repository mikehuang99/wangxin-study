<template>
  <div>
    <div>
      <course-list-header v-on:keyup.13="submits()" :reloadCourseList="reloadCourseList"></course-list-header>
      <div class="container full">
        <div class="search_criteria">
          <!-- 选项卡头部 -->
          <ul class="search_criteria_ul clearfix">
            <li class="search_criteria_tabs" >
              <a :class="{on: serachType == 'subject'}" href="javascript:" @click="activeSearchType('subject')">{{this.selectedSubjectName}}</a>
              <transition name="downlist">
                <div class="search_criteria_tab" id="courseSubject" v-show="serachType == 'subject'">
                  <div class="left_side">
                  ...
                  <div class="right_side">
                    <div class="right_side_i">
                      ...
                      <div class="right_two" v-for="(items,index) in subjectLevel.levelTwo" :key="index">
                        <a  :class="{active: isActive == index}" href="javascript:;" :data-id="items.levelTwoId" :data-pid="items.pid" class="third_menu" @click="chooseLevelTwo(index,items.levelTwoId)">{{items.twoSubjectName}}</a>
                        <div v-show="isActive == index">
                          <ul>
                            <li :data-id="items.levelTwoId" @click="chooseThree(items.twoSubjectName, items.levelTwoId, 2)"><nuxt-link to="/courseAllList">全部</nuxt-link></li>
                            <li  v-for="(item,indexs) in subjectLevel.levelThree" v-if="item.pid == items.levelTwoId" :key="indexs" :data-id="item.levelThreeId" @click="chooseThree(item.threeSubjectName,item.levelThreeId,3)"><nuxt-link to='/courseAllList'>{{item.threeSubjectName}}</nuxt-link></li>
                          </ul>
                        </div>
                      </div>
                    </div>
            ...
            <li class="search_criteria_tabs">
              <a :class="{on: serachType == 'sortType'}"  @click="activeSearchType('sortType')" href="javascript:" :sortByType = "this.sort">{{this.sort}}</a>
              <transition name="downlist">
                <div class="search_criteria_tab min_hieght" v-show="serachType == 'sortType'">
                  <ul class="localtion_order_list order_list">
            ...
            <li class="search_criteria_tabs">
              <a :class="{on: serachType == 'screen'}" @click="activeSearchType('screen')" href="javascript:">{{screenName}}</a>
              <transition name="downlist">
                <div class="search_criteria_tab" v-show="serachType == 'screen'">
                  <div class="tabbar screen_area">
                    <div class="main">
                      <div v-for="(item, indexs) in screen" :key="indexs">
                        <div class="screen_title">
                          <h3><span></span>{{item.title}}</h3>
                        </div>
                        <ul class="screen_detail">
                          <li v-for="(items,index) in item.screenTypeList" :key="items.id" :filter-id="items.id" @click="selectScreenType(indexs,index,items.value,items.screenType)">
                            <span :class="{on: screen[indexs].screenTypeList[index].status}">{{items.name}}</span>
                          </li>
                        </ul>
                      </div>
                    </div>
                    ...
                    <div class="screen_bottom">
                      <!--
                      <div class="screen_reset" data-role="reset" @click="reset()">重置</div>-->
                      <div class="screen_ok" @click="clickScreenOk()">关闭</div>
                    </div>
                  </div>
                </div>
              </transition>
          ...
          <transition name="showcover">
            <div class="back cover" v-show="serachType"></div>
          </transition>
        </div>
        <div class="course_list">
          <course-list :courseName="courseName" :sortType="sortType" :levelTwoId="levelTwoId" :levelThreeId="levelThreeId"
                        :selectedFeeType="selectedFeeType" :selectedCourseType="selectedCourseType"></course-list>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  import courseListHeader from '~/components/courseList/courseListHeader.vue'
