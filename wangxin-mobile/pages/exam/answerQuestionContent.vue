...
import singleChoice from '~/components/exam/answerQuestion/singleChoice.vue'
import multiChoice from '~/components/exam/answerQuestion/multiChoice.vue'
import judgeQuestion from '~/components/exam/answerQuestion/judgeQuestion.vue'
import fillBlank from '~/components/exam/answerQuestion/fillBlank.vue'
import simpleQuestion from '~/components/exam/answerQuestion/simpleQuestion.vue'
...
*/
      ...
      /*
       async cacheFavorites(favoritesStatus){
           let data;
           if(favoritesStatus=='0') {
               //data = await cacheFavoritesExamQuestion(this.courseDetail.id, this.questionId);
               ...
               //data = await delFavoritesExamQuestion(this.courseDetail.id, this.questionId);
         ...
         // if(data.status&&data.status==200) {
              this.$parent.reversalFavoritesStatus();
         // }
      },*/
      //处理错误题集和高频错误题集
    ...
    ...mapMutations([
      ...
      answerQuestion(status){
          ...
          this.$emit('setRightCount',answerRightArray==null?0:answerRightArray.length);
          let answerErrorArray = JSON.parse(getStore("answer_error_"+this.ccpId));
          this.$emit('setErrorCount',answerErrorArray==null?0:answerErrorArray.length);

          let data = getStore("examQuestions_"+this.ccpId);
          let dataArray = JSON.parse(data);    //转换为array对象
          this.continueStartIndex = getStore("continueStartIndex_"+this.ccpId);
          //alert("Number(this.continueStartIndex):"+Number(this.continueStartIndex));
          ...
          if(Number(this.continueStartIndex) <= dataArray.length){
              ...
              let countF = 0;
              if(favoritesArray!=null){
                  for(let f=0;f<favoritesArray.length;f++){

                      if(favoritesArray[f] == this.questionId){
                          countF++;
                      }
                  }
              }
              ...
              if(countF==0){
                  this.setFavoritesStatus('0');
              }else{
                  this.setFavoritesStatus('1');
              }*/
              //查看该题是否被收藏--结束
